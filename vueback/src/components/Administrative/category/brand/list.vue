  ...
  <el-table :data="tableData" style="width: 100%" @selection-change="selectItem">
    ...
    <el-table-column label="操作" width="200">
      <template scope="scope">
  				<div>
  					<span>
  						<router-link :to="{ name: 'brandEdit', params: { id: scope.row.id }}" class="btn-link edit-btn">
  						编辑
  						</router-link>
  					</span>
  					<span>
...
export default {
  ...
  methods: {
    ...
    confirmDelete(item) {
      this.$confirm('确认删除该品牌?', '提示', {
        ...
        this.apiDelete('admin/brand/', item.id).then((res) => {
    ...
    getBrand() {
      this.apiGet('admin/brand').then((res) => {
        this.handelResponse(res, (data) => {
          this.tableData = data
        })
      })
  ...
  computed: {
    addShow() {
      return _g.getHasRule('brand-save')
    },
    editShow() {
      return _g.getHasRule('brand-update')
    },
    deleteShow() {
      return _g.getHasRule('brand-delete')
    }
  },
  components: {
    btnGroup
